...
def login_user_camera(request):
	if request.method == 'POST':
		...
		if user.groups.filter(name="Estudiantes").exists():
			foto1 = UserProfile.objects.get(user=user).foto.url
...
def convertirse_en_partner(request):
	if request.method == 'POST':
		form_user = RegistrarUsuarioForm(request.POST)
	...
	return render(request, 'autenticacion/convertirse_en_partner.html', context)
